...
Public Module mPacPowerUCP1
    Public Class cPacPowerUPC1


        Public _Verbose As Boolean = False
        Public _VoltageASetPoint As String  'ASX is 2 Wire only...
        ...
        Public gTXRX_BUFFER(4095) As Byte
        ...
        Public WithEvents bgwPacPowerComm As BackgroundWorker
        Public IsConnected As Boolean = False
        Public Event BytesReceived(ByVal Message As String)
        ...
        'Private Sub bgwContScanUSB_DoWork(ByVal sender As System.Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles bgwPacPowerComm.DoWork
        ...
        ''' <param name="HandShaking">Serial handshaking type, hardware, software et, none etc...</param>
        ''' <returns>0 = Success, 1 = unable to init serial port</returns>
        ''' <remarks>Frank Boudreau 2012 Landis+Gyr, 2018</remarks>
        Public Function i_InitSerialPort(ByVal iBaudrate As Integer, ByVal Parity As Ports.Parity, ByVal StopBits As Ports.StopBits, ByVal iDataBits As Integer, ByVal strComPort As String, ByVal bDTREnable As Boolean, ByVal HandShaking As IO.Ports.Handshake, Optional ByVal Message As String = "") As Integer
            ...
            Catch ex As Exception
                Success = 1
                Message = "Unable to Configure Comport" + vbCrLf + ex.ToString
            End Try
        ...
        ''' </summary>
        ''' <param name="aBytes">Byte array of Data</param>
        ''' <param name="Message">Error Message if applicable</param>
        ''' <returns>0 = Success, 1 on error</returns>
        ''' <remarks></remarks>
        Public Function i_Transmit(ByRef abytes() As Byte, Optional ByVal Message As String = "") As Integer
            Dim Success As Integer = 0
            Try
                'Clear Buffers
            ...
            Catch ex As Exception
                Success = 1
                Message = "Unable to transmit message Comport" + vbCrLf + ex.ToString
            End Try
        ...
        ''' <summary>
        ''' Transmit routine for PAC Power Device UPC1/3 controller.
        ''' </summary>
        ''' <param name="TransmitMessage">Byte array of Data to send</param>
        ''' <param name="Message">Error Message if applicable</param>
        ''' <param name="AppendOPC_Query">Append Operation Complete Query to the message. 
        ''' The Device will return a '1' when all commands have been executed
        ''' This is useful for Hand shaking</param>
        ''' <param name="AppendCRLF">Append the Carriage Return and Line feed to the Command...</param>
        ''' <returns>0 = Success, 1 on error</returns>
        ''' <remarks></remarks>
        Public Function i_Transmit(ByVal TransmitMessage As String, Optional ByRef Message As String = "", Optional ByVal AppendOPC_Query As Boolean = True, Optional ByVal AppendCRLF As Boolean = True) As Integer
            Dim Success As Integer = 0

            Try
                Dim CurrentTime As Date = Now.AddMilliseconds(2000)

                While ReceiveBufferControl.MessageState <> eMessageState.IDLE
                ...
                'Append Operation Complete Query to Command
                If AppendOPC_Query Then
                    TransmitMessage = TransmitMessage + "; *OPC?"
                End If

                'Append Carriage Return Linefeed if needed...
                If AppendCRLF Then
                    TransmitMessage += vbCrLf
                End If

                'convert to byte array
                Dim abytes() As Byte = ASCIIEncoding.ASCII.GetBytes(TransmitMessage)
            ...
            Catch ex As Exception
                Success = 1
                Message = "Unable to transmit message to Comport" + vbCrLf + ex.ToString
            End Try
        ...
        ''' </summary>
        ''' <remarks>Frank Boudreau 2012 Landis+Gyr</remarks>
        Public Function i_Disconnect(Optional ByRef Message As String = "") As Integer
            Dim Success As Integer = 0 ' assume success
            Try
                If SerialPort.IsOpen Then
                    SerialPort.Close()
                End If
                IsConnected = False 'does not appear to be being used...FJB
            Catch ex As Exception
                Success = 1
                Message = "Unable to Disconnect Comport" + vbCrLf + ex.ToString
            End Try
            Return Success
        End Function
        ''' <summary>
        ...
        ''' </summary>
        ''' <remarks>Frank Boudreau 2012 Landis+Gyr</remarks>
        Public Function i_Connect(Optional ByRef Message As String = "") As Integer
            Dim Success As Integer = 0 ' assume success
            Try
                If SerialPort.IsOpen Then
                    SerialPort.Close()
                End If
                ' SerialPort.DtrEnable = False 'not sure why this is...FJB
                ' SerialPort.Handshake = Ports.Handshake.None 'not sure why this is...FJB
                'open port
                SerialPort.Open()
                'set time out
                ReceiveBufferControl.Ticks = My.Computer.Clock.TickCount

                Dim iTimeout As Integer = ReceiveBufferControl.Ticks + 500 '500mS
            ...
            Catch ex As Exception
                Success = 1
                Message = "Unable to Connect Comport" + vbCrLf + ex.ToString
            End Try
            Return Success
        End Function

        ''' <summary>
        ''' The Serialport class is not in in the Main GUI Thread
        ''' Raising event here to pass through to the GUI. The GUI will have to 
        ''' Declare a "Delegate" to interact in a thread safe manner...
        ''' </summary>
        ''' <remarks>Frank Boudreau 2018</remarks>
        Public Sub b_ReceiveData() Handles SerialPort.DataReceived
            ...
            Select Case ReceiveBufferControl.MessageState
                ...
                Case Else
                    ...
                    If gTXRX_BUFFER.Length < RX_BUFFER_SIZE Then
                        ReDim Preserve gTXRX_BUFFER(RX_BUFFER_SIZE)
                    End If
                    ...
                    SerialPort.Read(gTXRX_BUFFER, ReceiveBufferControl.Offset, ReceiveBufferControl.BytesToRead)
                    ReceiveBufferControl.Offset = ReceiveBufferControl.Offset + ReceiveBufferControl.BytesToRead

                    If ReceiveBufferControl.Offset > 0 And (gTXRX_BUFFER(0) And &HF) = CByte(&H6) Then ' It is a data packet
                        ReceiveBufferControl.MessageState = eMessageState.WaitingForLength
                    ElseIf ReceiveBufferControl.Offset > 0 And (gTXRX_BUFFER(0) And &HF) = CByte(&H3) Then 'Ack with no Data
                        If ReceiveBufferControl.Offset >= 4 Then
                            ReceiveBufferControl.MessageState = eMessageState.IDLE
                            ReDim Preserve gTXRX_BUFFER(ReceiveBufferControl.Offset - 1)
                            ReceiveBufferControl.Offset = 0
                        End If
                    ElseIf ReceiveBufferControl.Offset > 0 And (gTXRX_BUFFER(0) And &HF) = CByte(&HC) Then 'Ack with Delay
                        ReceiveBufferControl.MessageState = eMessageState.WaitingForLength
                    ElseIf ReceiveBufferControl.Offset > 0 And (gTXRX_BUFFER(0) And &HF) = CByte(&H9) Then 'Ack with Error
                        ReceiveBufferControl.MessageState = eMessageState.WaitingForLength
                    End If

                    If ReceiveBufferControl.MessageState = eMessageState.WaitingForLength Then
                        If ReceiveBufferControl.Offset >= 4 Then
                            ReceiveBufferControl.Length = CInt(gTXRX_BUFFER(3) Or (CUInt(gTXRX_BUFFER(2)) * 256)) + 6
                    ...
                    ReDim Preserve gTXRX_BUFFER(ReceiveBufferControl.Offset - 1)
                    Dim myString As String = str_Byte_Array_To_ASCII_String(gTXRX_BUFFER)
                    If myString.IndexOf("1" + vbCrLf) >= 0 Then
                        ReceiveBufferControl.MessageState = eMessageState.IDLE
                    End If

                    If myString Is Nothing Or myString.Trim = "" Then
                        'RaiseEvent BytesReceived("")
                        'do nothing
                    Else
                        RaiseEvent BytesReceived(myString)
                    End If
        ...
        ' ''' <remarks>Frank Boudreau 2012 Landis+Gyr</remarks>
        'Public Function i_Receive(ByRef LocalaBytes() As Byte, Optional ByVal Message As String = "") As Integer
        '    'assume success
        '    Dim Success As Integer = 0
        '    Try
        ...
        '    Catch ex As Exception
        '        Success = 1
        '        Message = "Error Receiving DAta" + vbCrLf + ex.ToString
        '    End Try
        '    Return Success
        'End Function
        'Class cPacket
        ...
        ''' <summary>
        ''' Class to encapsulate Identification Data From PAC Power, once parsed.
        ''' </summary>
        ''' <remarks></remarks>
        Class cPacPower_Identification
        ...
        Public Structure sChannelUShort
            ...
            Public C As UShort
        End Structure
        Public Identification As New cPacPower_Identification
        '
        ''' <summary>
        ''' Class to encapsulate Instant PAC Power Metric data once parsed 
        ''' </summary>
        ''' <remarks>Frank Boudreau 2018 Landis+Gyr</remarks>
        Class cMetricData
            'Instantaneous
            Public Volt_LN_RMS As Single
            Public VOLT_LL_RMS As Single
            Public Current_RMS As Single
            Public Current_Peak As Single
            Public Current_Crest_Factor As Single
        ...
        Public Class cMEASureCommands
            Const VOLT1 As String = ":MEAS:VOLT1?"
            Const VOLT2 As String = ":MEAS:VOLT2?"
            Const VOLT3 As String = ":MEAS:VOLT3?"
            Const VLL1 As String = ":MEAS:VLL1?"
            Const VLL2 As String = ":MEAS:VLL2?"
            Const VLL3 As String = ":MEAS:VLL3?"
            Const FREQUENCY As String = "MEAS:FREQ?"
            Const CURRENT1 As String = "MEAS:CURR:RMS1?"
            Const CURRENT2 As String = "MEAS:CURR:RMS2?"
            Const CURRENT3 As String = "MEAS:CURR:RMS3?"
            Const CURRENT_PEAK1 As String = "MEAS:CURR:PEAK1?"
            Const CURRENT_PEAK2 As String = "MEAS:CURR:PEAK2?"
            Const CURRENT_PEAK3 As String = "MEAS:CURR:PEAK3?"
            Const CURRENT_CREST1 As String = "MEAS:CURR:CREST1?"
            Const CURRENT_CREST2 As String = "MEAS:CURR:CREST2?"
            Const CURRENT_CREST3 As String = "MEAS:CURR:CREST3?"
            Const CURRENT_POWER1 As String = "MEAS:CURR:POW1?"
            Const CURRENT_POWER2 As String = "MEAS:CURR:POW2?"
            Const CURRENT_POWER3 As String = "MEAS:CURR:POW3?"
            Const CURRENT_KVA1 As String = "MEAS:CURR:KVA1?"
            Const CURRENT_KVA2 As String = "MEAS:CURR:KVA2?"
            Const CURRENT_KVA3 As String = "MEAS:CURR:KVA3?"
            Const CURRENT_PF1 As String = "MEAS:CURR:PF1?"
            Const CURRENT_PF2 As String = "MEAS:CURR:PF2?"
            Const CURRENT_PF3 As String = "MEAS:CURR:PF3?"
            Friend Function GetVoltage1() As String
                Return VOLT1
            End Function
            Friend Function GetVoltage2() As String
                Return VOLT2
            End Function
            Friend Function GetVoltage3() As String
                Return VOLT3
            End Function
            Friend Function GetVLL_1() As String
                Return VLL1
            End Function
            Friend Function GetVLL2() As String
                Return VLL2
            End Function
            Friend Function GetVLL1() As String
                Return VLL2
            End Function
        End Class
...
#Region "Measurement Commands"
            Const VOLT_ALL As String = ":MEAS:VOLT1?;VOLT2?;VOLT3?"
            Const VOLT1 As String = ":MEAS:VOLT1?"
            Const VOLT2 As String = ":MEAS:VOLT2?"
            Const VOLT3 As String = ":MEAS:VOLT3?"
            Const VLL_ALL As String = ":MEAS:VLL1?;VLL2?;VLL?"
            Const VLL1 As String = ":MEAS:VLL1?"
            Const VLL2 As String = ":MEAS:VLL2?"
            Const VLL3 As String = ":MEAS:VLL3?"
            Const FREQUENCY As String = "MEAS:FREQ?"
            ...
            Const CURRENT_CREST2 As String = "MEAS:CURR:CREST2?"
            Const CURRENT_CREST3 As String = "MEAS:CURR:CREST3?"
            Const POWER1 As String = "MEAS:CURR:POW1?"
            Const POWER2 As String = "MEAS:CURR:POW2?"
            Const POWER3 As String = "MEAS:CURR:POW3?"
            Const KVA1 As String = "MEAS:CURR:KVA1?"
            Const KVA2 As String = "MEAS:CURR:KVA2?"
            Const KVA3 As String = "MEAS:CURR:KVA3?"
            Const PF1 As String = "MEAS:CURR:PF1?"
            Const PF2 As String = "MEAS:CURR:PF2?"
            Const PF3 As String = "MEAS:CURR:PF3?"
            Friend CurrentCommand As String

            Friend Function GetVoltageAll() As String
                CurrentCommand = VOLT_ALL
                Return CurrentCommand
            End Function

            Friend Function GetVoltageA() As String
                CurrentCommand = VOLT1
                Return CurrentCommand
            End Function
            Friend Function GetVoltageB() As String
                CurrentCommand = VOLT2
                Return CurrentCommand
            End Function
            Friend Function GetVoltageC() As String
                CurrentCommand = VOLT3
                Return CurrentCommand
            End Function

            Friend Function GetVLL_ALL() As String
                CurrentCommand = VLL_ALL
                Return CurrentCommand
            End Function

            Friend Function GetVLLA() As String
                CurrentCommand = VLL1
                Return CurrentCommand
            End Function
            Friend Function GetVLLB() As String
                CurrentCommand = VLL2
                Return CurrentCommand
            End Function
            Friend Function GetVLLC() As String
                CurrentCommand = VLL3
                Return CurrentCommand
            End Function
            Friend Function GetFrequency() As String
                CurrentCommand = FREQUENCY
                Return CurrentCommand
            End Function

            Friend Function GetCurrentA() As String
                CurrentCommand = CURRENT1
                Return CurrentCommand
            End Function
            Friend Function GetCurrentB() As String
                CurrentCommand = CURRENT2
                Return CurrentCommand
            End Function
            Friend Function GetCurrentC() As String
                CurrentCommand = CURRENT3
                Return CurrentCommand
            End Function
            Friend Function GetCurrentPeakA() As String
                CurrentCommand = CURRENT_PEAK1
                Return CurrentCommand
            End Function
            Friend Function GetCurrentPeakB() As String
                CurrentCommand = CURRENT_PEAK2
                Return CurrentCommand
            End Function
            Friend Function GetCurrentPeakC() As String
                CurrentCommand = CURRENT_PEAK3
                Return CurrentCommand
            End Function

            Friend Function GetCurrentCrestA() As String
                CurrentCommand = CURRENT_CREST1
                Return CurrentCommand
            End Function
            Friend Function GetCurrentCrestB() As String
                CurrentCommand = CURRENT_CREST2
                Return CurrentCommand
            End Function
            Friend Function GetCurrentCrestC() As String
                CurrentCommand = CURRENT_CREST3
                Return CurrentCommand
            End Function

            Friend Function GetPowerA() As String
                CurrentCommand = POWER1
                Return CurrentCommand
            End Function
            Friend Function GetPowerB() As String
                CurrentCommand = POWER2
                Return CurrentCommand
            End Function
            Friend Function GetPowerC() As String
                CurrentCommand = POWER3
                Return CurrentCommand
            End Function

            Friend Function GetKiloVoltAmpsA() As String
                CurrentCommand = KVA1
                Return CurrentCommand
            End Function
            Friend Function GetKiloVoltAmpsB() As String
                CurrentCommand = KVA2
                Return CurrentCommand
            End Function
            Friend Function GetKiloVoltAmpsC() As String
                CurrentCommand = KVA3
                Return CurrentCommand
            End Function

            Friend Function GetPowerFactorA() As String
                CurrentCommand = PF1
                Return CurrentCommand
            End Function
            Friend Function GetPowerFactorB() As String
                CurrentCommand = PF2
                Return CurrentCommand
            End Function
            Friend Function GetPowerFactorC() As String
                CurrentCommand = PF3
                Return CurrentCommand
            End Function
#End Region
...
#Region "Source Commands"
            Friend Function SetVoltageAll(ByVal VoltageAmpltude As String) As String
                CurrentCommand = ":VOLT " + VoltageAmpltude
                Return CurrentCommand
            End Function

            Friend Function SetAndEnableVoltage1(ByVal VoltageAmpltude As String) As String
                CurrentCommand = ":VOLT1 " + VoltageAmpltude + "; OUTP ON"
                Return CurrentCommand
            End Function

            Friend Function SetVoltage1(ByVal VoltageAmpltude As String) As String
                CurrentCommand = ":VOLT1 " + VoltageAmpltude
                Return CurrentCommand
            End Function

            Friend Function SetVoltage2(ByVal VoltageAmpltude As String) As String
                CurrentCommand = ":VOLT2 " + VoltageAmpltude
                Return CurrentCommand
            End Function

            Friend Function SetVoltage3(ByVal VoltageAmpltude As String) As String
                CurrentCommand = ":VOLT3 " + VoltageAmpltude
                Return CurrentCommand
            End Function

            Friend Function SetFrequency(ByVal Frequency As String) As String
                CurrentCommand = ":FREQ " + Frequency
                Return CurrentCommand
            End Function

            Friend Function SetCurrentLimit(ByVal CurrentLimit As String) As String
                CurrentCommand = ":CURR:LIM " + CurrentLimit
                Return CurrentCommand
            End Function

            Friend Function GetCurrentLimit() As String
                CurrentCommand = ":CURR:LIM:MAX?"
                Return CurrentCommand
            End Function

            Friend Function SetCurrentProtection(ByVal Enable As Boolean)
                Dim Command As String = ":IPROT:STAT"
                If Enable = True Then
                    Command = Command + "1"
                Else
                    Command = Command + "0"
            ...
            Friend Function GetCurrentProtectionStatus() As String
                CurrentCommand = ":IPROT:TRIP"
                Return CurrentCommand
            End Function

            Friend Function ClearCurrentProtectionStatus() As String
                CurrentCommand = ":IPROT:CLE"
                Return CurrentCommand
            End Function

            Friend Function SetPhaseB(ByVal Phase As String) As String
                CurrentCommand = ":PHAS2 " + Phase
                Return CurrentCommand
            End Function

            Friend Function SetPhaseC(ByVal Phase As String) As String
                CurrentCommand = ":PHAS3 " + Phase
                Return CurrentCommand
            End Function

            Friend Function SetOutputToStoredWaveformAll(ByVal WaveformNumber As String) As String
                CurrentCommand = ":WAVEFORM " + WaveformNumber
                Return CurrentCommand
            End Function

            Friend Function SetOutputToStoredWaveform1(ByVal WaveformNumber As String) As String
                CurrentCommand = ":WAVEFORM1 " + WaveformNumber
                Return CurrentCommand
            End Function

            Friend Function SetOutputToStoredWaveform2(ByVal WaveformNumber As String) As String
                CurrentCommand = ":WAVEFORM2 " + WaveformNumber
                Return CurrentCommand
            End Function

            Friend Function SetOutputToStoredWaveform3(ByVal WaveformNumber As String) As String
                CurrentCommand = ":WAVEFORM3 " + WaveformNumber
                Return CurrentCommand
            End Function

            Friend Function SetRampTime(ByVal RampTime As Single) As String
                If RampTime > 300.0 Then
                    RampTime = 300.0
                End If

                If RampTime < 0.0002 Then
                    RampTime = 0.0002
                End If

                RampTime = Round(RampTime, 4)
                CurrentCommand = "RAMP " + String.Format("{0:##0.####}", RampTime.ToString)
                Return CurrentCommand

            End Function

            Public Function SetSourceImpedance(ByVal Impedance As String) As String
                CurrentCommand = "IMPED " + Impedance
                Return CurrentCommand
            End Function

            Public Function GetSourceImpedanceMinimum() As String
                CurrentCommand = "IMPED:LIM:MIN?"
                Return CurrentCommand
            End Function


            Public Function GetSourceImpedanceMaximum() As String
                CurrentCommand = "IMPED:LIM:Max?"
                Return CurrentCommand
            End Function

            ''' <summary>
            ''' Sets the output Form. ASX115 only supports 1
            ''' </summary>
            ''' <param name="FormNumber">1 = 2 Wire, 2 = 3 Wire, 3 = 4 Wire</param>
            ''' <remarks></remarks>
            Public Function SetForm(ByVal FormNumber As Integer)
                ...
                If FormNumber > 3 Then
                    FormNumber = 3
                End If
                CurrentCommand = ":FORM " + FormNumber.ToString
                Return CurrentCommand
            End Function

            Public Structure CouplingType
                Const DIRECT As String = "DIRECT"
                Const XFMR As String = "XFMR"
            End Structure

            Public Function SetCoupling(ByVal CouplingType As CouplingType) As String
                CurrentCommand = ":COUPL " + CouplingType.ToString
                Return CurrentCommand
            End Function
            ...
            Public Function SetVoltageALC(ByVal ALCState As TOGGLE_STATE) As String
                Dim strState As String = ""
                If ALCState = TOGGLE_STATE._ON Then
                    strState = "ON"
                Else
                    strState = "OFF"
                End If

                CurrentCommand = ":VOLT:ALC " + strState.ToString
                Return CurrentCommand
            End Function

            Public Enum eSensePath
                Internal = 0
                External = 1
            End Enum

            Public Function SetSensePath(ByVal SensePath As eSensePath) As String
                CurrentCommand = ":SENS:PATH " + SensePath.ToString
                Return CurrentCommand
            End Function

            Public Function SetOutputState(ByVal State As TOGGLE_STATE) As String
                Dim strState As String = ""
                If State = TOGGLE_STATE._ON Then
                    strState = "ON"
                Else
                    strState = "OFF"
                End If
                CurrentCommand = ":OUTP " + strState
                Return CurrentCommand
            End Function
...
#End Region
        ...
        Public SourceCommand As cCommands
        Public MeasureCommands As cMEASureCommands
        Public Sub Init()
            SourceCommand = New cCommands
            MeasureCommands = New cMEASureCommands
        End Sub
        ...
        ' ''' <remarks></remarks>
        'Public Function b_Parse_Radian_Data(ByVal Packet As cPacket) As Boolean
        ...
        '                    MetricData.Volt_LN_RMS = s_TI_to_VB_Single(Packet.Data)
        '                Case eInstantMetricsIndex2X.Amps
        '                    MetricData.Current_RMS = s_TI_to_VB_Single(Packet.Data)
        ...
        '                        j += 1
        '                        If i = 3 Then
        '                            MetricData.Volt_LN_RMS = s_TI_to_VB_Single(abyteLocal)
        '                            j = 0
        '                        ElseIf i = 7 Then
        '                            MetricData.Current_RMS = s_TI_to_VB_Single(abyteLocal)
        '                            j = 0
        '                        ElseIf i = 11 Then
        '                            MetricData.Power = s_TI_to_VB_Single(abyteLocal)
        '                            j = 0
        '                        ElseIf i = 15 Then
        ...
        '                    MetricData.Power = s_TI_to_VB_Single(Packet.Data)
