...
class MainWindow(QMainWindow, QThread):
    ...
    class ExportManager:
        ...
        def export_individual_meter_data(self):
            try:
                ...
                # --- ensure export dir exists / accessible ---
                try:
                    os.makedirs(export_dir, exist_ok=True)
                    test_path = os.path.join(export_dir, ".__write_test__")
                    with open(test_path, "w") as _t:
                        _t.write("test")
                    os.remove(test_path)
                except Exception as e:
                    QMessageBox.critical(self.main, "Export Error", f"Cannot write to selected folder:\n{export_dir}\n\n{e}")
                    return

                # --- build safe filename (sanitize invalid filename chars) ---
                data = test_channels[chan]
                metadata = data.get("metadata", {})
                meter_form = str(metadata.get("meter_form", "UNK"))
                serial_number = str(metadata.get("serial_number", "NA"))
                lv_hv = str(metadata.get("lv_hv", "NA"))
                bef_aft = str(metadata.get("bef_aft", "NA"))
                ...
                filename = f"{_safe_name(meter_form)}-{_safe_name(serial_number)}-{_safe_name(lv_hv)}-{_safe_name(bef_aft)}.csv"
                full_path = os.path.join(export_dir, filename)

                # --- overwrite check (AFTER full_path exists) ---
                if not self.check_overwrite(full_path):
                    print("Export canceled: user chose not to overwrite existing file.")
                    return
                ...
                QMessageBox.information(
                    self.main,
                    "Export Complete",
                    f"Data for channel {chan} exported to:\n{full_path}"
                )
    ...
    def __init__(self):
        ...
        self.ui.TB_stepTime.setText("10")
        ...
        # ✅ Default Source Correction to ON
        self.ui.cB_SourceCorrection.setChecked(True)
    ...
    def initialize_serial_connections(self):
        ...
        if hasattr(self, "serial_connections"):
            ...
            time.sleep(0.05)  # Allow OS to release handles
    ...
    def _prep_and_continue_ranging(self):
        ...
        if getattr(self, "_skip_full_setup", False) and not getattr(self, "_force_rearm_sources", False):
            self.append_log("Skip flag set → bypassing ranging prep.", "neutral")
            self.run_time_based_and_start_tests()
            return

        if self.test_aborted:
            self.append_log("Ranging prep skipped due to test abort.", "neutral")
            return
        ...
        # ---------- currents / mode flags ----------
        Ia = _f(getattr(self, "Ia_sec", 0.0))
        Ib = _f(getattr(self, "Ib_sec", 0.0))
        Ic = _f(getattr(self, "Ic_sec", 0.0))

        isIa_low = (0.0 < Ia <= 0.3)
        isIb_low = (0.0 < Ib <= 0.3)
        isIc_low = (0.0 < Ic <= 0.3)

        wasIa_low = bool(getattr(self, "isIa_Low", False))
        wasIb_low = bool(getattr(self, "isIb_Low", False))
        wasIc_low = bool(getattr(self, "isIc_Low", False))

        Ia_modeChanged = (wasIa_low != isIa_low)
        Ib_modeChanged = (wasIb_low != isIb_low)
        Ic_modeChanged = (wasIc_low != isIc_low)

        # ---------- 1) Clamp commands to RX via Arduino ----------
        if not _open_arduino_if_needed():
            return
        ...
        clamp_start_ms = 0
        clamp_gap_ms = 150
        clamp_done_ms = clamp_start_ms + (len(clam_plan) - 1) * clamp_gap_ms + 100

        for idx, (label, clam_idx, state) in enumerate(clam_plan):
            def send_clam(clam_idx=clam_idx, label=label, state=state):
                if self.test_aborted:
                    return
                try:
                    msg = f"scpi,RX,'CONF:MEAS:CLAM{clam_idx}:ENAB {state}',\n"
                    self.serial_port.write(msg.encode("utf-8"))
                    try:
                        self.serial_port.flush()
                    except Exception:
                        pass
                    self.append_log(f"{label} clamp → {state}", "neutral")
                except Exception as e:
                    self.append_log(f"{label} clamp send error: {e}", "negative")

            QTimer.singleShot(clamp_start_ms + idx * clamp_gap_ms, send_clam)

        # ---------- 2) Low/standard prep to each current source port ----------
        prep_channels = [
            ("Ia", Ia, Ia_modeChanged, isIa_low),
            ("Ib", Ib, Ib_modeChanged, isIb_low),
            ("Ic", Ic, Ic_modeChanged, isIc_low),
        ]

        # Timing knobs (safe / slower than your old “too fast” behavior)
        per_cmd_delay = 200
        port_cascade_offset = 250

        total_delay = clamp_done_ms  # don’t start source prep until clamp wave is done

        for j, (key, current_value, modeChanged, isLowNow) in enumerate(prep_channels):
            ...
            port_base_delay = total_delay + j * port_cascade_offset
            cmd_delay = 0

            # Only send prep if modeChanged or initial_step (your original intent)
            if (modeChanged or getattr(self, "initial_step", False)) and isLowNow:
                self.append_log(f"Sending low-current setup to {key}", "neutral")

                QTimer.singleShot(
                    port_base_delay + cmd_delay,
            ...
            elif (modeChanged or getattr(self, "initial_step", False)) and (not isLowNow):
                ...
                # Keep whatever your standard voltage is (you’ve used 150/115/100/85 in different places)
                # I’ll keep 100 here since you requested VOLT 100 for non-low-current behavior elsewhere.
                QTimer.singleShot(
                    port_base_delay + cmd_delay,
        ...
        # === 3) Continue AFTER everything is truly settled (this is the key fix) ===
        # Give the serial devices time to transmit + latch before we touch I2C (prevents Remote I/O error).
        POST_PREP_SETTLE_MS = 700
        QTimer.singleShot(
            int(total_delay + POST_PREP_SETTLE_MS),
            lambda: self._continue_ct_ranging() if not self.test_aborted else None
        )
    ...
    def _continue_ct_ranging(self):
        ...
        Critical behavior:
          - NO double-tap (single pass only)
          - Waits for current sources to be fully powered down (using send_source_commands(after=...))
            BEFORE doing CT_ranging_code I2C operations (prevents Remote I/O error)
        """
        from PyQt6.QtCore import QTimer
        ...
        tempIa_sec = getattr(self, "Ia_sec", 0.0)
        tempIb_sec = getattr(self, "Ib_sec", 0.0)
        tempIc_sec = getattr(self, "Ic_sec", 0.0)

        # If all currents are zero, skip CT setup and routing, jump to activate sources
        if float(tempIa_sec or 0.0) == 0.0 and float(tempIb_sec or 0.0) == 0.0 and float(tempIc_sec or 0.0) == 0.0:
            self.append_log(
                "All currents are zero. Skipping CT ranging and routing, jumping to activate sources.",
                "neutral",
            )
            QTimer.singleShot(150, lambda: self._continue_activate_sources() if not self.test_aborted else None)
            return
        ...
        self.Ib_sec = 0.0
        self.Ic_sec = 0.0
        self.append_log("Powering down current sources before CT ranging…", "neutral")

        def after_powerdown():
            if self.test_aborted:
                return

            # Step 2: Restore original current setpoints (we don't re-arm here; CT ranging comes first)
            self.Ia_sec = tempIa_sec
            self.Ib_sec = tempIb_sec
            self.Ic_sec = tempIc_sec

            # Step 3: CT ranging I2C work (single pass only)
            def continue_ct_setup():
                ...
                try:
                    ...
                    if float(self.Ia_sec or 0.0) > 0.0:
                        set_current("a", self.Ia_sec)
                    if float(self.Ib_sec or 0.0) > 0.0:
                        set_current("b", self.Ib_sec)
                    if float(self.Ic_sec or 0.0) > 0.0:
            ...
            # Small additional settle AFTER the power-down sequence completes, before I2C.
            QTimer.singleShot(600, continue_ct_setup)

        # Use the completion callback so we DON'T start I2C while SCPI writes are still in-flight.
        # extra_settle_ms here is "time after last SCPI write" before calling after_powerdown.
        self.send_source_commands(after=after_powerdown, extra_settle_ms=1200, hard_reset_currents=False)

    def _continue_activate_sources(self):
        """
        Activate sources ONCE (no double-tap), then continue pipeline.

        CHANGE:
          - Adds a hard 1 second delay AFTER sources are armed (after send_source_commands callback)
            BEFORE starting the initial IMET parsing/populate-table chain.
        """
        from PyQt6.QtCore import QTimer
        from PyQt6.QtSerialPort import QSerialPort
        ...
        # NEW: delay after arming completes before starting RX/IMET parsing
        PRE_PARSE_DELAY_MS = 1000
        ...
        def _after_arm():
            ...
            try:
                ...
                elif int(float(settle_text) * 1000) < 5000 and self.check_current_over_20A():
            ...
            def read_initial_metrics(phase_index=0, attempt=0):
                """Reads initial metrics from the standard for each phase and populates the metrics table.
                ...
                try:
                    if not self.serial_port.isOpen():
                        ...
                        if not self.serial_port.open(QSerialPort.OpenModeFlag.ReadWrite):
                            self.append_log(
                                f"ERROR: Could not open Arduino port for initial RX (Phase {phase_label})",
                                "negative",
                            )
                            if attempt == 0:
                    ...
                    def process_initial_response():
                        ...
                        try:
                            ...
                            self.append_log(
                                f"Initial IMET{imet_num} response for Phase {phase_label}: {response}",
                                "neutral",
                            )
                            ...
                            if comm_error or data_error:
                                self.append_log(
                                    f"Initial metric extraction failed for Phase {phase_label} (attempt {attempt + 1})",
                                    "negative",
                                )
                                if attempt == 0:
                                    QTimer.singleShot(250, lambda: read_initial_metrics(phase_index, attempt + 1))
                    ...
                    # keep this read window as you had it (you were at 1400ms in the later version)
                    QTimer.singleShot(1400, process_initial_response)
            ...
            # NEW: hard delay AFTER arming completes, BEFORE starting the settle timer / parsing chain
            self.append_log(f"Post-arm pacing: waiting {PRE_PARSE_DELAY_MS} ms before starting initial RX parsing…", "neutral")

            def _start_initial_parse_chain():
                if self.test_aborted:
                    return
                QTimer.singleShot(settle_delay_ms, lambda: read_initial_metrics())

            QTimer.singleShot(PRE_PARSE_DELAY_MS, _start_initial_parse_chain)

        # Arm once; pacing handled inside send_source_commands
        self.send_source_commands(after=_after_arm, extra_settle_ms=350, hard_reset_currents=False)

    def send_source_commands(self, *, after=None, extra_settle_ms=50, hard_reset_currents=False):
        """
        MainWindow-safe source activation with a REAL completion callback.

        FIXES / FEATURES:
          - Formats ANY numeric sent to sources with <=3 decimals (VOLT/CURR/PHAS).
          - No "double-tap" OUTP pulses.
          - For each current source (Ia/Ib/Ic) when ON and NOT low-current mode (I_sec > 0.3):
              • send "VOLT 115"
              • send "CURR:PROT:DEL 1"
              • then send the normal OUTP/CURR/PHAS command
          - Still staged + callback only after last scheduled write + settle.
        ...
        Params:
          ...
          hard_reset_currents: if True, forces Ia/Ib/Ic OFF first
        """
        from PyQt6.QtCore import QTimer
        import re
        ...
        # ---------- formatting helpers (MAX 3 decimals) ----------
        def _f3(x):
            """Return a string with <=3 decimals, stripping float-noise."""
            try:
                v = float(x)
            except Exception:
                v = 0.0
        ...
        def _a3(deg):
            """Angle formatted to <=3 decimals and normalized."""
            return _f3(self._norm_angle(deg))

        # ---------------- helpers used by existing logic ----------------
        ...
        # ---------------- build command strings (FORMATTED) ----------------
        cmds = {}

        Va_volt = 0.0 if float(getattr(self, "vA_pri", 0) or 0.0) == 0.0 else float(self.vA_pri)
        Vb_volt = 0.0 if float(getattr(self, "vB_pri", 0) or 0.0) == 0.0 else float(self.vB_pri)
        Vc_volt = 0.0 if float(getattr(self, "vC_pri", 0) or 0.0) == 0.0 else float(self.vC_pri)

        cmds["Va"] = f"OUTP 1;VOLT {_f3(Va_volt)};PHAS {_a3(0.0)};"
        cmds["Vb"] = f"OUTP 1;VOLT {_f3(Vb_volt)};PHAS {_a3(getattr(self, 'pab', 120.0))};"
        cmds["Vc"] = f"OUTP 1;VOLT {_f3(Vc_volt)};PHAS {_a3(getattr(self, 'pac', 240.0))};"

        Ia_sec = float(getattr(self, "Ia_sec", 0.0) or 0.0)
        Ib_sec = float(getattr(self, "Ib_sec", 0.0) or 0.0)
        Ic_sec = float(getattr(self, "Ic_sec", 0.0) or 0.0)

        iA_on = Ia_sec > 0.0
        iB_on = Ib_sec > 0.0
        iC_on = Ic_sec > 0.0

        # low-current mode is your <=0.3 A secondary behavior
        iA_low = (0.0 < Ia_sec <= 0.3)
        iB_low = (0.0 < Ib_sec <= 0.3)
        iC_low = (0.0 < Ic_sec <= 0.3)

        if iA_on:
            if iA_low:
                cmds["Ia"] = f"OUTP 1;VOLT {int(round(Ia_sec * 1000.0))};PHAS {_a3(getattr(self, 'pha', vA_target))};"
            else:
                self.Ia_pri = self.scale_current(Ia_sec)
                cmds["Ia"] = f"OUTP 1;CURR {_f3(self.Ia_pri)};PHAS {_a3(getattr(self, 'pha', vA_target))};"
        else:
            cmds["Ia"] = "OUTP 0;CURR 0;"

        if iB_on:
            if iB_low:
                cmds["Ib"] = f"OUTP 1;VOLT {int(round(Ib_sec * 1000.0))};PHAS {_a3(getattr(self, 'phb', vB_target if vB_target is not None else 120.0))};"
            else:
                self.Ib_pri = self.scale_current(Ib_sec)
                cmds["Ib"] = f"OUTP 1;CURR {_f3(self.Ib_pri)};PHAS {_a3(getattr(self, 'phb', vB_target if vB_target is not None else 120.0))};"
        else:
            cmds["Ib"] = "OUTP 0;CURR 0;"

        if iC_on:
            if iC_low:
                cmds["Ic"] = f"OUTP 1;VOLT {int(round(Ic_sec * 1000.0))};PHAS {_a3(getattr(self, 'phc', vC_target if vC_target is not None else 240.0))};"
            else:
                self.Ic_pri = self.scale_current(Ic_sec)
                cmds["Ic"] = f"OUTP 1;CURR {_f3(self.Ic_pri)};PHAS {_a3(getattr(self, 'phc', vC_target if vC_target is not None else 240.0))};"
        else:
        ...
        # Optional: hard-force currents off first
        ...
        # currents next (with the NEW extra commands when NOT low-current)
        base2 = base + 3 * step + 150
        current_keys = ("Ia", "Ib", "Ic")

        def _needs_std_prep(k: str) -> bool:
            if k == "Ia":
                return iA_on and (not iA_low)
            if k == "Ib":
                return iB_on and (not iB_low)
            if k == "Ic":
                return iC_on and (not iC_low)
            return False

        per_phase_gap = step

        for i, k in enumerate(current_keys):
            # each phase gets its own mini-sequence spaced by `step`
            t0 = base2 + i * per_phase_gap

            if _needs_std_prep(k):
                # 1) VOLT 115
                QTimer.singleShot(t0 + 0,   lambda kk=k: write_scpi(kk, "VOLT 115", "neutral"))
                # 2) CURR:PROT:DEL 1
                QTimer.singleShot(t0 + 60,  lambda kk=k: write_scpi(kk, "CURR:PROT:DEL 1", "neutral"))
                # 3) actual OUTP/CURR/PHAS command
                QTimer.singleShot(t0 + 120, lambda kk=k: write_scpi(kk, cmds[kk], "positive"))
                total_ms = max(total_ms, t0 + 120)
            else:
                # low-current mode OR OFF: just send the normal command for that phase
                QTimer.singleShot(t0, lambda kk=k: write_scpi(kk, cmds[kk], "positive"))
                total_ms = max(total_ms, t0)

        # done
        done_ms = int(total_ms + int(extra_settle_ms))
        if callable(after):
    ...
    def source_correction(self):
        ...
        IMET semantics:
          • v_ang_meas = absolute phase voltage angle (deg)
          • i_ang_meas = relative current angle (deg) = (I_abs − V_abs) mapped to (−180, 180]

        Targets from script:
          • Voltage targets (absolute):
              A = 0°
              B = table col 7 (default 120°)
              C = table col 8 (default 240°)
          • Current targets (absolute):
              A = col 12 if present else in-phase with A voltage target (0°)
              B = col 13 if present else in-phase with B voltage target
              C = col 14 if present else in-phase with C voltage target
            → For trimming, convert to desired relative: Δ_target = wrap((I_abs − V_abs), −180..180]

        Algorithm:
          Pass 1 (magnitudes): trim V & I magnitudes (skip if scripted magnitude == 0).
          Pass 2 (angles):
            1) Voltage (B,C only): incremental → err = wrap(V_target_abs − v_meas_abs); cmd := cmd + err
            2) Current (A,B,C):   incremental on relative → err_rel = wrap(Δ_target − i_meas_rel); cmd := cmd + err_rel
          Stop when |err| ≤ ±0.2° (PHASE_TOL_DEG) or attempts exhausted.
        """
        ...
        # Angle tolerances and attempts (tight for ±0.2°)
        PHASE_TOL_DEG = 0.2
        ...
        def _wrap_err_abs(measured_abs, target_abs):
            """Signed error for absolute angles: (target - measured) in (−180, 180]."""
        ...
        def _slew_limit(old, target, max_abs, max_pct):
            ...
            if max_pct is not None: cap = min(cap, abs(old) * max_pct)
            if cap != float('inf'):
        ...
        def _read_imet_parsed(imet_num):
            """Return (p, v_mag, i_mag, v_ang_abs, i_ang_rel). NOTE: i_ang_meas is RELATIVE to V."""
            try:
                response = ""
                while self.serial_port.canReadLine():
                ...
                v_abs = float(vals[3]); i_rel = float(vals[4])  # relative: (I − V)
                i_rel = _wrap_pm180(i_rel)
                return p, v, i, v_abs, i_rel
            except Exception:
                return None

        # Absolute V target (deg) from script
        def _v_target_abs(phase_label):
            if phase_label == 'A':
                return 0.0
            elif phase_label == 'B':
                val = _cell_float(self.current_step, 7, 120.0)  # pab
                return self._norm_angle(0.0 if val is None else val)
            else:
                val = _cell_float(self.current_step, 8, 240.0)  # pac
                return self._norm_angle(0.0 if val is None else val)

        # Absolute I target (deg) from script; blank → in-phase with V target
        ...
        # Magnitudes from script or *_sec fallback
        ...
        def pass1_correct_phase(idx):
            ...
            if idx >= len(phases):
                ...
                QTimer.singleShot(300, lambda: pass2_correct_phase(0)); return

            phase_label, imet_num, *_rest, voltage_col, _ = phases[idx]
            attempts = {"v": 0, "i": 0}
            stage = {"s": "V"}

            def tick(): _kick_imet_read(imet_num, on_ready)
            ...
            def on_ready(_=None):
                ...
                if parsed is None:
                    ...
                    QTimer.singleShot(200, lambda: pass1_correct_phase(idx + 1)); return
                ...
                if stage["s"] == "V":
                    ...
                    if v_err <= v_tol or attempts["v"] >= 2:
                        ...
                        stage["s"] = "I"; QTimer.singleShot(250, tick); return
                    ...
                    QTimer.singleShot(500, tick); return

                # skip I magnitude if scripted 0
                if exp_i <= 0.0:
                    self.append_log(f"{phase_label} I skipped (scripted 0).", "neutral")
                    QTimer.singleShot(250, lambda: pass1_correct_phase(idx + 1)); return
        ...
        # ===== PASS 2: Angles (Voltage absolute; Current relative-to-V) =====
        def pass2_correct_phase(idx):
            ...
            if idx >= len(phases):
                ...
                self.read_final_imet_snapshot(); return
            ...
            # Desired RELATIVE offset (I−V), mapped to (−180,180]
            delta_target = _wrap_pm180(I_target_abs - V_target_abs)

            # -------- Voltage loop: trim to ABSOLUTE (B,C only; skip A or V=0) --------
            def trim_voltage():
                if self.test_aborted: return
            ...
            # -------- Current loop: trim RELATIVE (I−V) to delta_target --------
            def trim_current():
                if self.test_aborted: return
                ...
                def _after_read(_=None):
                    ...
                    if parsed is None:
                        if attempts["iang"] < I_ANG_ATTEMPTS:
                            ...
                            QTimer.singleShot(400, lambda: pass2_correct_phase(idx + 1)); return

                    _, _, _, v_meas_abs, i_meas_rel = parsed  # i_meas_rel is (I−V)
            ...
            # Start voltage trim for this phase, then relative-current trim
            trim_voltage()
    ...
    def read_final_imet_snapshot(self):
        ...
        def read_imet_sequential(phase_index=0):
            ...
            if phase_index >= len(phases):
                ...
                QTimer.singleShot(150, lambda: not self.test_aborted and self.check_pause_time())
                return
            ...
            try:
                ...
                QTimer.singleShot(250, lambda: read_imet_sequential(phase_index + 1))
                return
    ...
    def run_time_based_and_start_tests(self):
        ...
        QTimer.singleShot(250, lambda: not self.test_aborted and self.send_next_test_command())
    ...
    def send_next_test_command(self):
        ...
        def send_next():  # do not remove this indentation
            ...
            if not channels_queue:
                ...
                self.serial_port.flush()
                QTimer.singleShot(1200, lambda: not self.test_aborted and self._send_run_and_start_polling())
                return
    ...
    def _prep_and_continue_jog(self):
        ...
        for key, current_value in prep_channels:
            ...
            if current_value <= 0.3:
                ...
                delay_counter += delay_step
                QTimer.singleShot(delay_counter, lambda p=port, k=key: (self.append_log(f"{k} → VOLT 115", "neutral"), p.write(b"VOLT 115\n")))
                delay_counter += delay_step
